...
func (n *node) addRoute(path, routeName string, handlers HandlersChain) {
	fullPath := path
	n.priority++
	numParams := countParams(path)
	// Empty tree
	if len(n.path) == 0 && len(n.children) == 0 {
		n.insertChild(numParams, path, routeName, fullPath, handlers)
		n.nType = root
		return
	}
...
func (n *node) insertChild(numParams uint8, path, routeName string, fullPath string, handlers HandlersChain) {
	for numParams > 0 {
